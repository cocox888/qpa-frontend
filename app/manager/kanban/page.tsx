...
import KanbanTaskCreateModal from '@/components/modal/managerModal/kanbanTaskCreateModal';
...
const KanbanBoard: React.FC = () => {
    ...
    if (e.target.value !== '') {
      ...
      setColumns((prevColumns) =>
        prevColumns.map((column) => ({
          ...
          tasks: kanbanTasks.filter((item, index) => {
            return (
              item.status == column.id && item.title?.includes(e.target.value)
            );
          })
        }))
      );
  ...
  const handleDrop = (e: DragEvent<HTMLDivElement>, targetColumnId: string) => {
    ...
    api
      .post(`${role}/updateKanbanTaskStatusById`, {
        task_id: taskId,
      ...
      .then(() => {
        ...
        dispatch(
          updateKanbanTaskStatusById({ id: taskId, status: targetColumnId })
        );
      })
      .catch((e) => {
        console.log(e);
  ...
  const handleTaskUpdate = (updatedTask: KanbanTask) => {
    ...
    const role = localStorage.getItem('role');
    api
      .post(`${role}/updateKanbanTaskById`, updatedTask)
      .then(() => {
  ...
  const handleTaskDelete = (taskId: number) => {
    api
      .delete(`/manager/kanbanTask/${taskId}`)
      .then((res) => {
        setColumns((prevColumns) =>
          prevColumns.map((column) => ({
            ...column,
              ...
              <form
                onSubmit={(e) => {
                  ...
                  const updatedTask = {
                    ...
                    label: formData.get('labels')?.toString()
                    // .split(',')
