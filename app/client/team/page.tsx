...
export default function Team() {
  ...
  const filterData = () => {
    ...
    switch (filter) {
      case 0:
        ...
        break;
      case 1:
        result = membersArray.filter((item) => item.status === '1');
        break;
      case 2:
        result = membersArray.filter((item) => item.role === 'admin');
        break;
      case 3:
        result = membersArray.filter((item) => item.role === 'manager');
        break;
      default:
        result = membersArray.filter((item) => item.role === 'member');
        break;
    }
    return result;
  };
  ...
  useEffect(() => {
    ...
    setActiveProjectsNumber(
      projects.filter((item) => item.state !== 'Completed').length
    );
    setActiveTasksNumber(
      projects.reduce(
        (count, project) =>
          count +
          (project?.projectTask?.filter((item) => item.state !== 'Completed')
            ?.length || 0),
        0
      )
    );
    ...
    const userAssigned = projects.reduce((acc: any, project) => {
      // Loop through each assigned user in the project
      project?.assignedProjectUser?.map((user) => {
        console.log(user);
        totalMember += 1;
        if (user.status === '1') {
          activeCount += 1;
        ...
        acc[user?.id || 0].totalAssignedTime += 1;
      });
      console.log(acc);
      return acc;
    }, {}); // <-- This empty object `{}` is the initial value of `acc`
    setAllMembers(totalMember);
